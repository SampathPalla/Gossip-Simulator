...
case class GossipMessage (network_node:ActorRef, selectedAction:String, noOfNodes:Int, failureNodes : Int) extends Message
case class Initialize (loop_id:Int, neighbors:List[Int], network_nodes:List[ActorRef], master:ActorRef) extends Message
case class Terminate (id:Int, gossipCount:Int, start_time:Long, failureNodes : Int) extends Message

//GOSSIP CLASS
case class Action_Gossip (id:Int, start_time:Long, failureNodes_List : List[Int]) extends Message
// PUSH SUM CLASS
case class Action_PushSum (sum:Double, weight:Double, start_time:Long, failureNodes_List : List[Int]) extends Message
...
object Project2Bonus extends App {
  ...
  var selectedNoOfFailNodes = 0       //THIS IS THE NUMBER OF DESIRED FAILURE NODES

  if (args.length == 4) {
    noOfNodes = args(0).toInt
    selectedTopolgy = args(1).toString()
    selectedAction = args(2).toString()
    selectedNoOfFailNodes = args(3).toInt
    ...
    master ! GossipMessage(node(0), selectedAction, noOfNodes, selectedNoOfFailNodes)
  }
  //MASTER CLASS
  class Master extends Actor{
    ...
    def receive={
      case GossipMessage (network_node:ActorRef, selectedAction:String, noOfNodes:Int, failureNodes : Int) => {

        var failureNodes_List : List[Int] = Nil
        var temp : Int = 0
        var i : Int = 0
        var count : Int = 0
        while(i < failureNodes){

          temp = Random.nextInt(noOfNodes)
          if(temp < noOfNodes && temp > 0)
          {
            failureNodes_List ::= temp
            count += 1
          }
          i += 1
        }
        println("LIST OF NODES THAT ARE FAILING RANDOMLY: " + failureNodes_List)
        ...
        if(selectedAction == "GOSSIP"){
          network_node ! Action_Gossip (-1, start_time, failureNodes_List)
        }
        else
        if(selectedAction == "PUSHSUM"){
          network_node ! Action_PushSum (0, 1, start_time, failureNodes_List)
        }
        else{
      ...
      case Terminate (id:Int, gossipCount:Int, start_time:Long, failureNodes:Int) => {
        ...
        if(Nodes_Visited.length == Total_Nodes - failureNodes) {

          println("LIST OF NODES VISITED : ")
          var i:Int = 0
          while (i < Nodes_Visited.length) {
  ...
  class Nodes extends Actor{
    ...
    def receive={
      ...
      case Action_Gossip(callerId:Int, start_time:Long, failureNodes_List : List[Int]) => {
        if (RumourCount < 10 && (!(failureNodes_List contains LoopID))) {

          if (callerId != LoopID) {
            RumourCount += 1
            master ! Terminate(LoopID, RumourCount, start_time, failureNodes_List.length)
          }

          RandomNeighbor = Random.nextInt(neighbors_List.length)
          node(neighbors_List(RandomNeighbor)) ! Action_Gossip(LoopID, start_time, failureNodes_List)
          self ! Action_Gossip(LoopID, start_time, failureNodes_List)
      ...
      case Action_PushSum(new_sum:Double, new_weight:Double, start_time:Long, failureNodes_List : List[Int]) => {

        if(!(failureNodes_List contains LoopID)){
          ...
          if ((RumourCount == 1) || (Math.abs((oldratio-newratio)) > math.pow(10, -10))) {
            Delta_Difference=0
            var RandomNeighbor = Random.nextInt(neighbors_List.length)
            node(neighbors_List(RandomNeighbor)) ! Action_PushSum(sum, weight, start_time, failureNodes_List)
          }
          else {
